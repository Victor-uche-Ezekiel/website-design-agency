...
export function ProjectContent({ project }: ProjectContentProps) {
  ...
  return (
    <div className={`min-h-screen ${isDark ? "bg-[hsl(240,10%,3.9%)]" : "bg-gray-50"}`}>
      {/* Hero Section */}
      <section ref={headerRef} className="relative min-h-[85vh] flex items-center">
      ...
      <section className={`py-24 border-b ${isDark ? "border-white/10" : "border-gray-200"}`}>
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            <div>
              <h3 className={`text-lg font-semibold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Client URL</h3>
              <a
            ...
            <div>
              <h3 className={`text-lg font-semibold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Sector</h3>
              <p className={`${isDark ? "text-gray-400" : "text-gray-600"}`}>{project.sector}</p>
            </div>
            <div>
              <h3 className={`text-lg font-semibold mb-2 ${isDark ? "text-white" : "text-gray-900"}`}>Type</h3>
              <p className={`${isDark ? "text-gray-400" : "text-gray-600"}`}>{project.type}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section ref={contentRef} className="py-24">
        <div className="container mx-auto px-4">
          ...
          >
            {/* Objective */}
            <div>
              <h2 className={`text-3xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-900"}`}>The Objective</h2>
              <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-600"}`}>{project.objective}</p>
            </div>

            {/* Process */}
            <div>
              <h2 className={`text-3xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-900"}`}>The Process</h2>
              <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-600"}`}>{project.process}</p>
            </div>

            {/* Result */}
            <div>
              <h2 className={`text-3xl font-bold mb-6 ${isDark ? "text-white" : "text-gray-900"}`}>The Result</h2>
              <p className={`text-lg ${isDark ? "text-gray-400" : "text-gray-600"}`}>{project.result}</p>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      {/* Testimonial Section */}
      {project.testimonial && (
        <section className={`py-24 ${isDark ? "bg-black/10" : "bg-gray-50/80"}`}>
          <div className="container mx-auto px-4">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              animate={contentInView ? { opacity: 1, y: 0 } : {}}
      ...
      >
        <div className="container mx-auto px-4">
          <motion.div
            ...
            transition={{ duration: 0.8 }}
          >
            <h2 className={`text-2xl font-bold mb-12 text-center ${isDark ? "text-white" : "text-gray-900"}`}>
